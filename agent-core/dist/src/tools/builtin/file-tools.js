...
export function createNodeFileOperations(cwd = process.cwd()) {
    const safe = (path) => safeResolve(cwd, path, "file path");
    return {
...
export function createReadTool(operations) {
    return defineTool({
        name: "read",
        ...
        async execute({ params, operations }) {
            ...
            if (offset < 1)
                throw new Error("offset must be 1-indexed and >= 1");
            if (limit < 1)
                throw new Error("limit must be >= 1");
            const data = await operations.readFile(params.path);
            const mimeType = imageMimeType(params.path);
            if (mimeType)
                return { content: [{ type: "image", mimeType, data: data.toString("base64") }], details: { path: params.path, offset, limit, truncated: false, mimeType } };
...
export function createEditTool(operations) {
    return defineTool({
        name: "edit",
...
export function createWriteTool(operations) {
    return defineTool({
        name: "write",

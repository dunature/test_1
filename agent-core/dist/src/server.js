...
import { AgentEventHub, AgentSession, AuthStorage, createBacktestTool, createBridgeServer, createDefaultResourceOperations, createDockerRqalphaOperations, createEditTool, createFetchDataTool, createLoadResourceTool, createNodeFileOperations, createReadTool, createSessionApi, createTushareOperations, createWriteTool, ModelRegistry, OpenAICompatibleAdapter, ToolRegistry, } from "./index.js";
...
function createTools(userId) {
    ...
    if (process.env.RQALPHA_DATA_PATH)
        backtestOptions.dataPath = process.env.RQALPHA_DATA_PATH;
    registry.register(createBacktestTool(createDockerRqalphaOperations(backtestOptions), { userId }));
    return registry;
}
function createRuntimeSession(tools, sessionId) {
    ...
    if (!model)
        throw new Error("DeepSeek model configuration is missing");
    const requestAuth = {};
    if (process.env.DEEPSEEK_API_KEY)
        requestAuth.apiKey = process.env.DEEPSEEK_API_KEY;

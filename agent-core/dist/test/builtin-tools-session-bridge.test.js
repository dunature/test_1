...
test("read/edit/write tools support pagination, exact replacement, parent mkdir", async () => {
    ...
    try {
        ...
        registry.register(createWriteTool(ops));
        registry.register(createReadTool(ops));
        registry.register(createEditTool(ops));
...
test("fetch_data uses replaceable provider operations and offset truncation", async () => {
    ...
    assert.equal(result.result?.details && result.result.details.truncated, true);
...
test("load_resource returns metadata and preview for files", async () => {
    ...
    try {
        ...
        const registry = new ToolRegistry();
        registry.register(createLoadResourceTool({ load: async (params) => ({ metadata: { id: "x", source: params.source, uri: params.uri, name: "note.md", size: 12, sha256: "abc", summary: "# title" }, contentPreview: "# title" }) }));
        const result = await registry.execute({ id: "l", name: "load_resource", arguments: { source: "file", uri: "note.md" } });
        assert.equal((result.result?.details).summary, "# title");
    }
    finally {
        await rm(dir, { recursive: true, force: true });
    }
...
test("createBridgeServer rejects static path traversal", async () => {
    ...
    try {
        ...
        await writeFile(join(publicDir, "index.html"), "ok", "utf8").catch(async (error) => {
...
test("WebSocketClient parses inbound masked text frames and routes messages", async () => {
    ...
    client.onMessage = (raw) => resolveMessage(JSON.parse(raw).text);
...
test("AgentEventHub creates one server-side session per WebSocket client", () => {
    const hub = new AgentEventHub();
    const a = new WebSocketClient(new FakeSocket());
    const b = new WebSocketClient(new FakeSocket());
    const makeSession = (client) => new AgentSession({ provider: {}, model: {}, auth: {}, sessionId: client.sessionId });
...
function maskedClientTextFrame(text) {

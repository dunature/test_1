...
import { SandboxManager, type DockerRunSpec, type DockerRunner } from "../src/index.js";

class CapturingRunner implements DockerRunner {
  specs: DockerRunSpec[] = [];
  constructor(private readonly hold?: Promise<void>) {}
  async run(spec: DockerRunSpec): Promise<{ stdout: string; stderr: string }> {
...
test("SandboxManager enforces max concurrent container leases", async () => {
  ...
  await assert.rejects(
    manager.run({ userId: "u2", image: "img", command: ["true"], files: [] }),
    /sandbox pool exhausted: 1\/1/,
  );
  release();
  await first;
...
test("SandboxManager isolates user data roots and rejects traversal", async () => {
  ...
  await assert.rejects(
    manager.run({ userId: "user-a", image: "img", command: ["true"], files: [], dataPath: "../user-b" }),
    /escapes root/,
  );
  await rm(dir, { recursive: true, force: true });
});

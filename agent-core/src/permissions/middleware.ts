...
export const permissionConfigs: Record<PermissionRole, PermissionConfig> = {
  readonly: {
    role: "readonly",
    defaultMode: "deny",
    rules: [
...
export function createPermissionMiddleware(role: PermissionRole, override?: Partial<PermissionConfig>): PermissionMiddleware {
...
export function decision(mode: "allow" | "ask" | "deny", reason?: string): PermissionDecision {
  const out: PermissionDecision = { mode };
